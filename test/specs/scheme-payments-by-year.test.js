...
test.describe('Scheme payments by year page', () => {
  ...
  test('Should have a download link', async ({ page }) => {
    const downloadLink = page.locator('#download-scheme-payments-by-year-summary-link')

    await expect(downloadLink).toHaveText('Download this data (.CSV)')
    await expect(downloadLink).toHaveAttribute('href', '/download-scheme-payments-by-year-summary')
  })
